        ...
        if name == "Go":
            ...
            try:
                ...
                if is_https:
                    ...
                    _info("HTTPS detected — panel listens on localhost:8443")
                    _hint("Point your reverse proxy at 127.0.0.1:8443")
                else:
                    ports = '      - "80:80"'
                    ssl_env = ""
...
def action_client_setup(cfg: Config) -> None:
    """Generate Prism Launcher pre-launch scripts with the pack URL baked in."""
    ...
    if not pack_url:
        ...
        if not pack_url:
            _fail("No pack URL configured")
            _hint("Use 'CDN setup' first, or set PACK_HOST_URL in settings")
            return
        cfg.set_global("PACK_HOST_URL", pack_url)

    pack_url = pack_url.rstrip("/")
    dest = click.prompt("  Output directory", default=str(Path.cwd()))
    out = Path(dest)
    ...
    _hint("Copy these scripts into your Prism instance's .minecraft folder")


def action_client_info(cfg: Config) -> None:
    ...
    if pack_url:
        ...
        console.print()
        _hint("Players point packwiz-installer at this URL")
        _hint("Generate scripts: use 'Generate client scripts' above")
    else:
        _warn("No PACK_HOST_URL configured")
        _hint("Use 'CDN setup' first, or set it in Settings")
...
SECTIONS: list[tuple[str, str, list[tuple[str, Any]]]] = [
    ...
    ("CDN", _MAGENTA, [
        ...
        ("Generate client scripts", action_client_setup),
        ("Show pack URL", action_client_info),
    ]),
    ("Dependencies", _BLUE, [
