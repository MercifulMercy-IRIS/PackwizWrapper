#!/usr/bin/env bash
# Prism Launcher pre-launch script — packwiz-installer bootloader
# Place this in your Prism instance directory and set it as the pre-launch command.
#
# Usage in Prism Launcher:
#   Settings > Custom Commands > Pre-launch command:
#     bash $INST_DIR/prism-update.sh
#
# Configuration: set PACK_URL below to your hosted pack.toml URL.

PACK_URL="${PACK_URL:-__PACK_URL__}"
BOOTSTRAP_JAR="packwiz-installer-bootstrap.jar"
BOOTSTRAP_URL="https://github.com/packwiz/packwiz-installer-bootstrap/releases/latest/download/packwiz-installer-bootstrap.jar"

cd "$INST_MC_DIR" || exit 1

# Download the bootstrap jar if missing
if [ ! -f "$BOOTSTRAP_JAR" ]; then
    echo "[ELM] Downloading packwiz-installer-bootstrap..."
    curl -fsSL -o "$BOOTSTRAP_JAR" "$BOOTSTRAP_URL" || {
        echo "[ELM] Failed to download bootstrap jar"
        exit 1
    }
fi

echo "[ELM] Updating modpack from $PACK_URL..."
java -jar "$BOOTSTRAP_JAR" -g -s client "$PACK_URL/pack.toml"
